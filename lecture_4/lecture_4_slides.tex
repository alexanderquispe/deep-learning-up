...
\begin{frame}
\frametitle{Recap from Lecture 3}

\textbf{What we learned:}
\begin{itemize}
    \item \textbf{Adam} optimizer: momentum + adaptive LR + bias correction.
    \item \textbf{Initialization} matters: He (ReLU), Xavier (sigmoid/tanh).
    \item \textbf{Regularization}: L1 (sparsity), L2 (weight decay), dropout.
    \item \textbf{Batch normalization}: normalize activations per batch.
    \item \textbf{UAT}: depth is exponentially more efficient than width.
\end{itemize}

\vspace{8pt}
\textbf{Today's question:}
\begin{center}
\textcolor{red}{\Large How do we apply neural networks to \textbf{images} and other spatial data?}
\end{center}

\vspace{6pt}
\textbf{Answer:} \textbf{Convolutional Neural Networks (CNNs)} --- networks that exploit spatial structure.
\end{frame}
...
\begin{frame}
\frametitle{Why CNNs Matter for Economists}

\textbf{Three reasons:}

\vspace{8pt}
\begin{enumerate}
    \item \textbf{Unstructured data is everywhere.} Satellite imagery, street views, scanned documents, X-rays in health economics, etc. CNNs unlock these for econometric analysis.

    \vspace{6pt}
    \item \textbf{Filling data gaps.} In developing countries, traditional surveys are sparse. CNNs on satellite data can fill gaps at \textbf{high spatial resolution} and \textbf{low cost}.

    \vspace{6pt}
    \item \textbf{Causal inference + DL.} CNNs can be combined with double machine learning, instrumental variables, and other causal frameworks to estimate treatment effects in non-tabular settings.
\end{enumerate}

\vspace{8pt}
\begin{center}
\textcolor{red}{\textbf{Caveat:} interpretability and identification remain challenges. CNNs are excellent at \emph{prediction}, but \emph{causal inference} requires careful design.}
\end{center}
\end{frame}
